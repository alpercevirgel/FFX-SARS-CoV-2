...
```{r}
IDs_samples <- sample_IDs_FFX %>%
  pull()

IDs_subjects <- sample_IDs_FFX %>% 
    mutate(subject_identifier = as.numeric(str_sub(as.character(samples_si), 1, -2))) %>%
  dplyr::select(subject_identifier) %>%
  pull() %>%
  unique()
ffx_subject %>%
  filter(sample_identifier %in% IDs_samples) %>%
  ...
  summarise(
    ...
    min_age = min(age),
    max_age = max(age),
    number_female = sum(sex == 'female', na.rm = TRUE),
    percentage_female = round(mean(sex == 'female', na.rm = TRUE) * 100, 1),
    number_mild = sum(severity_text == 'mild', na.rm = TRUE),
    percentage_mild = round(mean(severity_text == 'mild', na.rm = TRUE) * 100, 1),
    number_moderate = sum(severity_text == 'moderate', na.rm = TRUE),
    percentage_moderate = round(mean(severity_text == 'moderate', na.rm = TRUE) * 100, 1),
    number_severe = sum(severity_text == 'severe', na.rm = TRUE),
    percentage_severe = round(mean(severity_text == 'severe', na.rm = TRUE) * 100, 1),
    percentage_total = percentage_mild+percentage_moderate+percentage_severe
  ) 
...
# number of adult subject_identifier
ffx_subject %>%
  filter(subject_identifier %in% IDs_subjects) %>% filter(adult_child %in% "adult") %>% select(subject_identifier) %>% pull() %>% unique()

# number of adult subject_identifier
ffx_subject %>%
  filter(subject_identifier %in% IDs_subjects) %>% filter(adult_child %in% "child") %>% select(subject_identifier) %>% pull() %>% unique()
```
...
```{r}
flowsom_df_wide %>%
    dplyr::select(subject_identifier,timepoint,severity_text,comp3_PRR,comp4_PPP,sex,CD4_4) %>%
  filter(comp3_PRR %in% "TRUE")

sub_df <- flowsom_df_wide  %>%
  dplyr::select(subject_identifier,timepoint,comp3_PRR,comp4_PPP,sex,contains(c("CD4_","CD8_","B_","Mono_","NK_"))) %>%
  filter(timepoint %in% c("1","2","3")) %>%
  droplevels() %>%
  as.data.frame() 

vars_to_plot <- c("CD4_4","CD4_5","CD8_11","Mono_2","B_1","B_9")
...
for (var in vars_to_plot) {
  ...
  sub_df_1 <- sub_df[sub_df$comp4_PPP == "TRUE", ]
  sub_df_2 <- sub_df[sub_df$comp3_PRR == "TRUE", ]
  sub_df_1 <- sub_df_1 %>%
    mutate(shape_val = ifelse(sex == "male", 4, 15))
  
  p <- ggplot() +
    geom_half_boxplot(data = sub_df_2, aes(x = timepoint, y = !!sym(var), group = timepoint, fill = "grey"), 
                      outlier.shape = NA, center = FALSE, errorbar.draw = FALSE, width = .4, position = position_nudge(0.13)) +
    geom_point(data = sub_df_1, aes(x = timepoint, y = !!sym(var), group = subject_identifier, color = "#F5793A", shape = shape_val), size = 1.5, show.legend = FALSE) +
    geom_line(data = sub_df_1, aes(x = timepoint, y = !!sym(var), group = subject_identifier, color = "#F5793A"), size = 0.5) +
    geom_half_boxplot(data = sub_df_1, aes(x = timepoint, y = !!sym(var), group = timepoint, fill = "#F5793A"), 
                      outlier.shape = NA, center = FALSE, errorbar.draw = FALSE, width = .4, position = position_nudge(-0.13)) +
    theme_classic() +
    theme(legend.position = "none") +
    scale_fill_identity(guide = "none") +
    scale_shape_identity()  # Add this line to apply shapes directly
...
ggsave(paste0(PPP_save.dir, "combined_plots.pdf"), combined_plot, width =6, height = 3.5)
```
